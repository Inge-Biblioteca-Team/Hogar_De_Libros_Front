...
const RoomCardForCarrousel = ({ Rooms }: { Rooms: Room }) => {
  return (
    <div className=" w-full gap-8 justify-between dark:text-black bg-white flex rounded-md h-full space-x-2 text-right">
      <span className="!bg-white w-3/4 m-3">
        <h3 className="text-2xl font-bold max-md:text-base ">{Rooms.name}</h3>
      ...
      <img
        src={Rooms.image[0]}
        alt={Rooms.name}
        className="w-11/12 max-lg:w-1/2"
      />
    </div>
  );
};
