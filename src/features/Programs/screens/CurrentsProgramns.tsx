  ...
  return (
    <>
      {Programs && Programs?.count > 0 && (
        <section
          className="flex h-full md:w-full  items-center max-sm:w-full max-sm:h-full lg:gap-8 w-4/5 flex-col max-sm:m-0 "
          id="Programs"
        >
          ...
          <Carousel
            ...
            leftControl
            rightControl
            className="w-[64rem] lg:w-[80rem] md:w-full md:h-[30rem] md:pl-2 md:pr-2  max-sm:w-full max-sm:h-[500px] "
          >
            {groupedProgram.map((group, groupIndex) => (
              <div key={groupIndex} className=" flex md:gap-4 md:h-full md:w-full justify-center max-sm:h-full lg:h-full  max-sm:pr-4 max-sm:pl-4 lg:gap-x-4">
                {group.map((program) => (
                  <CardProgram
